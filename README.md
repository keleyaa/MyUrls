...
只暴露应用端口，Redis 仅在内部网络可达。兼容性与安全边界详见
...
curl --fail-with-body http://localhost:8080/short \
  ...
  --data-urlencode 'shortKey=docs-demo'
...
发布工作流只在 `v*` 发布标签或手动运行时构建 `ghcr.io/keleyaa/myurls`。它发布版本
标签和完整 Git commit SHA 标签，不发布或承诺 `latest` 标签。使用前先在仓库 Packages
页面确认目标标签已经存在，例如：

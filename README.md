...
- **镜像发布**：推送扁平的 `v*` Git 标签或手动运行时，GHCR 都会先完成完整 CI，再构建
  `ghcr.io/keleyaa/myurls`。稳定 `vX.Y.Z` 会更新 `latest`，使其始终指向最新稳定版本；使用前请在 Packages 页面确认标签。
...
- **运行要求**：Go `1.25.0`（建议使用固定的 `go1.26.6` toolchain）、Redis 7.4 或 8.x、
